...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!names) {
            message.channel.send(embed.setDescription(`${member} İsimli Kullanıcının  ismi \`${name} ${config.registration.symbol} ${age}\` olarak değiştirildi ve <@&${config.registration.onewoman}> rolü verilerek kayıt edildi!\n\n <@${message.author.id}> Tarafından Kaydedildi Toplamda **${toplam}** Kaydını Gerçekleştirdi`))
        } else {
            channel.send(embed.setDescription(`Kullanıcı başarıyla <@${message.author.id}> İsimli Yetkili Tarafından <@&${config.registration.onewoman}> Rolleri Verilerek kayıt edildi!\n\nKullanıcının toplamda " ${names.length} " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`))
        }
