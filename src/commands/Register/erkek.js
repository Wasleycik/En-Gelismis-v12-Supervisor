...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        const member = message.mentions.users.first() || guild.members.cache.get(args[0]) || author;
        const name = args[1]
        const age = args[2]
        ...
        if (!names) {
            channel.send(embed.setDescription(`${member} İsimli Kullanıcının  ismi \`${name} ${config.registration.symbol} ${age}\` olarak değiştirildi ve <@&${config.registration.oneman}> rolü verilerek kayıt edildi!\n\n <@${message.author.id}> Tarafından Kaydedildi Toplamda **${toplam}** Kaydını Gerçekleştirdi`))
        } else {
            channel.send(embed.setDescription(`Kullanıcı başarıyla <@${message.author.id}> İsimli Yetkili Tarafından <@&${config.registration.oneman}> Rolleri Verilerek kayıt edildi!\n\n Kullanıcının toplamda " ${names.length} " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`))
        }

        client.channels.cache.get(config.channels.chat).send(`Aramıza Hoşgeldin ${member} Seni Gördümüze Sevindik :D`);
    }
}